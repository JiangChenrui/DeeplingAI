...
def fc(input, name, n_out, p):
    ...
    with tf.name_scope(name) as scope:
        ...
        biases = tf.Variable(
            tf.constant(0.1, shape=[n_out], dtype=tf.float32), name="biases")
...
def time_tensorflow_run(session, target, feed, info_string):
    num_setps_burn_in = 10
    total_duration = 0.0
    total_duration_squared = 0.0

    for i in range(num_batches + num_setps_burn_in):
        start_time = time.time()
        _ = session.run(target, feed_dict=feed)
        duration = time.time() - start_time
        if i >= num_setps_burn_in:
            if not i % 10:
                print('%s: step %d, duration = %.3f' %
                      (datetime.now(), i - num_setps_burn_in, duration))
